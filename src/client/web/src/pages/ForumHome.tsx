// Page forum : navigation type Reddit avec flux vide tant que l'API n'alimente rien.
import { useMemo, useState } from 'react'
import { Input } from '../components/ui/Input'
import { SidebarItem, SidebarList } from '../components/composite/SidebarList'
    ...
    <>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          <Button variant="outline" className="rounded-full px-5 py-3" onClick={() => setCreatePostModalOpen(true)}>
            Créer un post
          </Button>
        </div>
          ...
          <Card className="max-h-[60vh] overflow-y-auto pr-1">
            <CardHeader className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => setAddCommunityModalOpen(true)}>
                  Ajouter
                </Button>
            ...
            <Card className="border border-border bg-white/90 shadow-sm">
              ...
              <CardContent className="space-y-6">
                ...
                <p className="text-base text-muted">
                  {activePost.excerpt} {activePost.excerpt}
                </p>
                    ...
                    >
                      <article className="relative flex gap-4 px-6 py-5">
                        <div className="flex-1 space-y-2">
                          ...
                          <p className="text-sm text-muted">{post.hasImage ? 'Contenu avec aperçu visuel' : post.excerpt}</p>
                        </div>
                        <div className="flex flex-shrink-0 items-center justify-center">
                          <div
                            className={`${
                              post.hasImage ? 'h-32 w-32' : 'h-24 w-24'
                            } rounded-2xl bg-gradient-to-br from-primary/20 via-primary/5 to-background-soft`}
                          />
                        </div>
                      </article>
                    </button>
...
function CreatePostModal({ open, onClose, communities, activeCommunityId }: CreatePostModalProps) {
...
function AddCommunityModal({ open, onClose }: AddCommunityModalProps) {
  return (
    <Modal
