// Messagerie : dispositions sans données mock (en attente d'API).
...
/**
 * Messaging page containing the conversation list and current thread.
 * @returns JSX Element describing the messaging experience.
 */
export function Messages() {
  const [threads] = useState<{ id: string; title: string }[]>([]) // TODO API: hydrater via GET /api/messages/threads.
  const [messagesByThread] = useState<Record<string, Message[]>>({}) // TODO API: remplacer par les messages renvoyés pour chaque thread.
      ...
      <section className="flex min-h-[75vh] flex-col rounded-2xl border border-border bg-white shadow-sm">
        <header className="border-b border-border px-6 py-5">
          <h2 className="text-lg font-semibold text-foreground">
            {getConversationTitle(activeConversation, threads)}
          </h2>
...
/**
 * Resolve the conversation title from the available threads.
 * @param id active thread identifier
 * @param threads available threads
 * @returns thread title or a default label
 */
function getConversationTitle(id: string | null, threads: { id: string; title: string }[]) {
